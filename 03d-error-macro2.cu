...
#if defined(NDEBUG)
#define CUDA_CHECK_KERNEL_ERROR()	0
#else
#define CUDA_CHECK_KERNEL_ERROR()	do { \
        cudaError_t e = cudaGetLastError(); \
        if (cudaSuccess != e) { \
            printf("cuda kernel failure \"%s\" at %s:%d\n", \
                   cudaGetErrorString(e), \
                   __FILE__, __LINE__); \
            exit(1); \
		} else { \
            printf("cuda kernel success at %s:%d\n", \
                   __FILE__, __LINE__); \
        } \
    } while (0)
#endif
...
// HISTORY: error-macro2.cu <-- error-macro.cu <-- error-check.cu <-- get-symbol.cu <-- vec-inc-static.cu
static const char rcsid[] __attribute__((used)) = "$Id: 03d-error-macro2.cu,v 1.2 2026/08/29 02:07:37 wayfarecru Exp $";
